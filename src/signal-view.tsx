...
export function SignalWindow({engine,children}:{engine:TapeEngine;children:React.ReactNode}){
  ...
  return <div className={`signal-window ${mode!=='tape'?'analysing':''}`}>
    <div className="signal-tabs" role="group" aria-label="Band window display">{(['tape','spectrum','history'] as Mode[]).map(value=><button key={value} aria-pressed={mode===value} onClick={()=>setMode(value)}>{value==='tape'?'Band':value==='spectrum'?'Spectrum':'History'}</button>)}</div>
