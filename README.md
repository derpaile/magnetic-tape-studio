...
Open the local URL. New sessions start with four empty lanes. Open a lane’s Library to choose an original sound, drop your own sample directly onto a lane, or record your microphone. Existing saved sessions are restored. Use Record master to capture the performance and export WAV, FLAC or M4A files.
...
- The live context requests 48 kHz and interactive latency for microphone monitoring. Browsers and audio devices choose the actual round-trip delay; the separate eight-second master capture buffer remains in place. Samples decode on a separate offline context, native buffers are reused, and padded/reversed import buffers are copied in small yielding slices before the audible crossfade. Library synthesis runs in its own worker.
...
## Live input, spectral inspection and export

- Choose **Live microphone** in a lane’s input strip; open **Choose input** for device, mono/left/right/stereo channel, ±24 dB input gain and optional four-beat count-in. Input settings are remembered, but microphone access and monitoring never restart automatically after reload. A removed input is reported explicitly.
- Live input can be monitored independently of tape playback or microphone recording. It passes through its assigned lane’s volume, pan, mute, solo and echo send. Existing tape on that lane still plays for overdubs. Selecting another lane does not move an active input or a recording’s destination. Choose **Audio on tape** on the live lane to disconnect it. The lane’s waveform area becomes an input display with peak/clip feedback. Use headphones for monitoring and count-in.
- The count-in is scheduled on the audio clock and excluded from the recorded microphone samples. Its audible clicks go only to the listening output. Microphone takes remain dry, after input gain/channel selection; master takes include the effects. A mono input is duplicated when a right or stereo channel is requested.
- **Band / Spectrum / History** share the reel glass. Analysis reads the microphone, selected lane before effects, or master after effects, using 8192-point frequency analysis, a logarithmic 20 Hz–20 kHz axis and a fixed −96 to 0 dBFS scale. **Freeze** holds the display, **Reference** captures a spectrum, and the pointer reveals frequency/level. Displays use the mono sum; stereo cancellation can therefore reduce a reading. Solo a lane to inspect its contribution to the shared effects.
- The analysis window’s **A / B** button auditions the original mix at measured RMS matched to the effects output (bounded to ±12 dB). It uses the same running tape/loop and does not change effect settings. Matching is measured when switching to A, not a perceptual loudness measurement. Master capture forces B and disables audition switching.
- Master takes can be renamed and exported as **WAV (16/24-bit)**, **FLAC (16/24-bit lossless PCM)** or **M4A (AAC at 128/192/256 kb/s)**. The export dialog provides filename, range, estimate, progress and cancellation. Range edits are remembered without changing the original; playback/bounce still use the original full take. Converting an old 16-bit take to 24-bit does not restore detail absent from its source.
- **Finish with effect tail** pauses tape playback and silences new monitored input, then saves when the output has been quiet for 1.5 seconds (after a minimum 2 seconds), or after 30 seconds. Existing Stop & save remains an immediate stop. Frozen/self-oscillating effects reach the 30-second ceiling; tails absent from an already saved take cannot be reconstructed during export.
- WAV preparation uses bounded reads in a worker, preserving stereo chunk order across trimmed boundaries. FLAC/AAC use a pinned, single-thread FFmpeg WebAssembly converter hosted with the app. It loads on first compressed export (about 31 MB), is cached for offline reuse, and terminates after every export/cancel. WORKERFS reads the prepared Blob on demand. Long compressed exports still require memory for the encoded output; a failure leaves the original take intact.
- `scripts/prepare-codecs.mjs` creates reproducible converter assets before build/dev. The WASM is split into assets below Cloudflare’s 25 MiB limit and fetched only when needed, so installing the studio does not wait for the converter. The package lock pins the converter; generated files are not committed. FFmpeg core is GPL-2.0-or-later; its source/build and license links ship in the converter NOTICE.

Additional checks: `tests/export.mjs` verifies cross-chunk trims, 16/24/float WAV samples, legacy preservation and sample-accurate count-in. `tests/live-export.mjs` exercises real browser microphone routing, independent selection, device removal, spectral controls, desktop/mobile layout, decodable WAV/FLAC/AAC output, trimming, persistence and offline AAC export.
