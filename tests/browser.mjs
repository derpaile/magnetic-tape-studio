...
try{
  ...
  const downloadEvent=page.waitForEvent('download');await page.getByRole('button',{name:'Export latest take',exact:true}).click();await page.getByRole('button',{name:'Download file',exact:true}).click();const exported=await downloadEvent;await exported.saveAs('test-results/master.wav');
  ...
  await page.getByRole('button',{name:'Loop track 4',exact:true}).click();await page.getByRole('button',{name:'2 beats',exact:true}).click();await page.getByRole('slider',{name:'Selected track echo send',exact:true}).fill('0.3');await page.getByRole('slider',{name:'Selected track pan',exact:true}).fill('-0.4');await page.getByRole('slider',{name:'Head 3 time',exact:true}).focus();await page.keyboard.press('ArrowUp');const savedHead3=await page.getByRole('slider',{name:'Head 3 time',exact:true}).getAttribute('aria-valuenow');await page.getByRole('slider',{name:'Back to tape',exact:true}).focus();await page.keyboard.press('ArrowUp');await page.getByLabel('Session name',{exact:true}).fill('A saved experiment');await page.waitForTimeout(1000);await page.reload();await page.getByText('Saved on this device',{exact:true}).waitFor();assert.equal(await page.getByLabel('Session name',{exact:true}).inputValue(),'A saved experiment');await page.getByRole('button',{name:'Select track 3: Microphone take + overdub',exact:true}).waitFor();await page.getByRole('button',{name:'Select track 4: Glass bells',exact:true}).waitFor();await page.getByRole('button',{name:'Export latest take',exact:true}).waitFor();
